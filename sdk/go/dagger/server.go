...
func (ts *goTypes) schema() []byte {
	...
	for _, s := range ts.Structs {
		...
		*/
		if strings.HasPrefix(s.typ.PkgPath(), "go-vanity-test.netlify.app/dagger/sdk/go/dagger") {
			continue
		}
...
func goReflectTypeToGraphqlType(t reflect.Type, isInput bool) *ast.Type {
	...
	case reflect.String:
		...
		in-progress codegen work.
		*/
		if strings.HasPrefix(t.PkgPath(), "go-vanity-test.netlify.app/dagger/sdk/go/dagger") {
